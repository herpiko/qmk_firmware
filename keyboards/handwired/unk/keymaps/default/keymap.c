...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_QWERTY] = LAYOUT(
    ...
    LOWER,      KC_LCTL,   KC_LALT, KC_LGUI, LOWER,   KC_MS_BTN1,                                         KC_SPACE,  RAISE,   KC_RALT, KC_RCTL,    KC_DELETE,  KC_PGDOWN
  ),
  [_COLEMAK] = LAYOUT(
    KC_ESC,     KC_GRV,    KC_1,    KC_2,    KC_3,    KC_4,     KC_5,   KC_6,                  KC_7,    KC_8,      KC_9,    KC_0,    KC_MINS,    KC_EQL,     KC_BSPACE, 
  ...
  [_LOWER] = LAYOUT(
    GAMING, _______, KC_F1,   KC_F2,   KC_F3,    KC_F4,   KC_F5,   KC_F6,                     KC_F7,   KC_F8,   KC_F9,    KC_F9,   KC_F10,  KC_F11,  KC_F12,
    ...
    COLEMAK_CO, _______, _______, _______, _______,  _______, _______,                        _______,    KC_PGUP,    _______,    KC_PGDOWN,   _______,    _______, KC__VOLUP, 
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case COLEMAK_CO:
      if (record->event.pressed) {
        layout = 2;
        set_single_persistent_default_layer(_COLEMAK_CO);
      }
      return false;
    case GAMING:
      if (record->event.pressed) {
...
void oled_task_user(void) {
    ...
    if (layout == 0) {
        ...
        oled_write_P(PSTR("Layout: COLEMAK"), false);
    } else if (layout == 2) {
        oled_write_P(PSTR("Layout: CO/COLEMAK"), false);
    } else if (layout == 3) {
        oled_write_P(PSTR("Layout: GAMING"), false);
    }
